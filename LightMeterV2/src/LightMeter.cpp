//
// Copyright 2018 marshalltaylorSFE & Dashie <dashie@sigpipe.me>
//
...
// Boing boing
void cfgPushButton(Bounce &bB) {  // NOLINT
 ...
 */
void onButtonReleased(Button &btn, uint16_t duration) {  // NOLINT
...
void LightMeter::getLuxAndCompute(bool fstop) {
    ...
    if (fstop) {
        ...
        oled.drawExposureStyle2(exposureTable[ConfigUser.exposureSetting], 2,
                                13);
...
void LightMeter::process(void) {
    ...
    uiPbUp.update();
    uiPbDown.update();
    uint32_t curEncoderPos = uiEncoder.read();
    ...
    switch (state) {
        ...
        case MDisplayPhotoValueInit:
            ...
            drawDisplay = true;
            oled.clear(PAGE);
            oled.drawMenu("Photo", isoTable[ConfigUser.isoSetting], false,
                          true);
        ...
        case MDisplayPhotoValue:
            ...
            if (curEncoderPos != lastEncoderPos) {
                ...
                } else if (curEncoderPos >=
                           lastEncoderPos + ROT_ENC_DETENTS_COUNTS) {
                    // moved up
        ...
        case MSystemInit:
            ...
            oled.setCursor(64, 11);
            oled.print(static_cast<float>(lipo.voltage()) / LIPO_CAPACITY, 2);
            oled.print("V");

            oled.setCursor(0, 20);
            oled.print(static_cast<float>(lipo.soc()), 0);
            oled.print("%");
        ...
        case MMary:
            drawDisplay =
                true;  // might need special handling since it was handled
