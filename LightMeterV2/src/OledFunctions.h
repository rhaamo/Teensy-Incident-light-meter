// Copyright 2017 marshalltaylorSFE
#ifndef LIGHTMETERV2_SRC_OLEDFUNCTIONS_H_
#define LIGHTMETERV2_SRC_OLEDFUNCTIONS_H_

#include <Arduino.h>
#include "TeensyView.h"

// Comes from LightMeter
 ...
 * logo.png
 * 128x32, bg black, invert colors, code output "Arduino code, single bitmap",
 * draw vertical
 */
static uint8_t screenlogo[LCDMEMORYSIZE] = {
    // 'logo, 128x32px
    ...
    ,
...
class OLEDFunctions : public TeensyView {
 public:
    OLEDFunctions(uint8_t rst, uint8_t dc, uint8_t cs, uint8_t sck,
                  uint8_t mosi)
        : TeensyView(rst, dc, cs, sck, mosi) {}
    void drawByte(uint8_t x, uint8_t y, uint8_t c) {
        uint8_t color = WHITE;
        uint8_t mode = NORM;
        for (int i = 0; i < 8; i++) {  // 8 is the LCD's page height (see
    ...
    void drawLogo(void) {
        drawBitmap(screenlogo);
        display();
    }
    void drawSeperator1(void) { line(0, 8, 127, 8); }
    ...
    void drawMenu(const char *name, bool lArrow, bool rArrow) {
        ...
        print(name);
        // Force widget
        batteryStyle1(116, 0, static_cast<float>(lipo.soc()) / 100);
    ...
    void drawMenu(const char *name, uint16_t isoValue, bool lArrow,
                  bool rArrow) {
    ...
    void drawFNumStyle2(float fNumIn, uint8_t xIn, uint8_t yIn) {
        // yIn is always 1 decimal
        setCursor(xIn, yIn);
        print("F");
        setFontType(1);
        line(xIn + 4, yIn + 10, xIn + 9, yIn + 2, WHITE, NORM);
...
#endif  // LIGHTMETERV2_SRC_OLEDFUNCTIONS_H_
